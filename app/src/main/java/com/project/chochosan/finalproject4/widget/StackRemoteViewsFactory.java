...
public class StackRemoteViewsFactory implements RemoteViewsService.RemoteViewsFactory {
    ...
    public void onCreate() {
        cursor = context.getContentResolver().query(DatabaseContract.FavoriteColumns.CONTENT_URI, null, null, null, null);
    }
    ...
    public RemoteViews getViewAt(int position) {
        ...
        if (cursor.moveToPosition(position)) {
            ...
            Bitmap bp = null;
            String posterPath = data.getPoster();
            try {
                bp = Picasso.with(context).load(alamatGambar+ posterPath).get();
...
//                bp = Picasso.with(context).load(alamatGambar + data.getPosterPath()).get();
        ...
        extras.putInt(ImageViewWidget.EXTRA_ITEM, position);
